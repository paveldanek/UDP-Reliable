...
public class CLI implements UI {
	...
	private String[] receiveCodes = { "RECV", "DUPL" };
	private String[] errorStatuses = { "SENT", "DROP", "ERRR" };
	private String[] conditions = { "RECV", "!Seq", "CRPT" };
	...
	public InitSettings getSettings(String[] args) throws UnknownHostException {
		...
		// if no agrs (including the mandatory output file name) were specified
		if (args.length < 1 || args[0].equalsIgnoreCase("-?") || args[0].equalsIgnoreCase("-h")) {
			println("Arguments expected.\nFirst (mandatory) argument: File (incl. path) to write into. \n"
	...
	// printing methods, called by corresponding Server Context methods
	...
	public void printReceive(int receiveCode, int seqNo, double receiveTime, int condition) {
		System.out.println("[" + receiveCodes[receiveCode] + "] " + seqNo + " " + String.format("%.3f", receiveTime)
				+ "ms [" + conditions[condition] + "]");
	}

	@Override
	public void printAck(int sentCode, int ackNo, int errorStatus) {
		System.out.println("[" + sentCodes[sentCode] + "] " + ackNo + " [" + errorStatuses[errorStatus] + "]");
	}
